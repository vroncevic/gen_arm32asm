...
Info
    Defines class GenARM32ASM with attribute(s) and method(s).
...
class GenARM32ASM(ATSCli):
    '''
        ...
        It defines:
            ...
            :methods:
                | __init__ - Initials GenARM32ASM constructor.
                | process - Processes and runs tool operation.
    '''

    _GEN_VERBOSE: str = 'GEN_ARM32ASM'
    _CONFIG: str = '/conf/gen_arm32asm.cfg'
    _LOG: str = '/log/gen_arm32asm.log'
    _LOGO: str = '/conf/gen_arm32asm.logo'
    _OPS: List[str] = ['-n', '--name', '-v', '--verbose']

    def __init__(self, verbose: bool = False) -> None:
        '''
            Initials GenARM32ASM constructor.
        ...
        gen_arm32asm_property: Dict[str, str | bool] = {
        ...
        Splash(gen_arm32asm_property, verbose)
        ...
        if self.is_operational():
            self.add_new_option(
                ...
                help='generate ARM 32-bit asm project'
            )
            self.add_new_option(
        ...
        if self.is_operational():
            try:
                ...
                if exists(f'{getcwd()}/{str(getattr(args, "name"))}'):
                    ...
                    return status
                gen: GenARM32ASMPro = GenARM32ASMPro(verbose=verbose)
                try:
                    print(
                        " ".join([
                            f'[{self._GEN_VERBOSE.lower()}]',
                            'generate ARM 32-bit asm project skeleton',
